...
\section{Cathode Ray}
	...
	\begin{itemize}
		\item In the presence of a magnetic field, the ray will deflect
		\item Due to its deflection, it must be a negatively charged particle
		\item Paris of electric plates cause the rays to bend towards the positive plate
		\item The paddle-wheel experiment shows that it has momentum, and therefore has mass
		\item Changes the colour of silver salts
		\item Can pass through thin metal foil without damaging them
	\end{itemize}

\section{Mass of Oil Droplets}



Bohr used Planck's quantum theory to hypothesise that each band on the emission spectrum must correspond to a particular energy level

Energy must be released form the atom in quantum amounts which resulted in the signature bands on the spectrum

The energy emitted and absorbed must relate to the electrons' orbit or state

Bohr studied the hydrogen spectrum because of the simplicity of the hydrogen atom

\section{Spontaneous Decay}
	
	\begin{itemize}
		\item Americium is a good alpha particle emitter
	\end{itemize}
