...
\section{Natural Polymers}
	...
	\subsection{Proteins}
		...
		Amino acids undergo condensation polymerisation to form an amide in the same way as nylon. The link that joins two amino acids together is called a peptide link (and is the same as the amide link in polyamides).
...
\section{Ultraviolet-visible Spectrophotometry}

	UV-vis spectrophotometry is useful for determining the concentration of both inorganic and organic molecules, as well as metal ions. Therefore, it is useful in determining rates of reactions and equilibrium constants. This method is similar to atomic absorption spectroscopy (AAS) and colourimetry.

	UV-vis spectrophotometry uses the range from 190-800 nm. As with AAS and colourimetry, UV-vis relies on the electrons in the molecules being excited to a higher energy state.

	When the appropriate quantised light photon hits the atom, the electron will enter a higher energy state

	Most organic chemicals are colourless, so they do not absorb in the visible region of the spectrum. However many natural and synthetic dyes and pigments have energy transitions that do absorb in the visible region. The group of atoms responsible for UV–vis absorption are called chromophores.

	Deuterium or tungsten lamps are used because they produce appropriate wavelengths of light

	The light is focused onto a diffraction grating that splits it into different wavelengths.

	This light is shone through two cells simultaneously, one through the sample, and one through the reference cell containing the solvent

	The detector will record the wavelengths at which absorption occurs.

	Absorbance is measured as:

	\begin{align*}
		A = \log_10{\frac{I_0}{I}}
	\end{align*}

	The absorbance is related to the concentration of the solution. The greater the concentration, the greater the absorbance since there are more molecules or ions to absorb the light. The Beer-Lambert law states the quantitative relationship between absorbance and concentration; $A = \epsilon lc$, where $\epsilon$ is the molar absorbtivity of the sample, $l$ is the length of light passing through the sample usually designed to 1 cm. $c$ is the concentration of the sample. Absorbance is a ratio and does not have units.

	UV-vis spectrophotometry can be used to:

	\begin{itemize}
		\item confirm the identity of the substance by comparing the spectrum of a sample to the spectrum of a pure sample or standard of the compound
		\item calculate concentrations of a substance using a calibration curve of known concentrations of analyte
			\begin{itemize}
				\item This can be used to measure the concentration of reactants over time to determine the rate of reaction inside a cell
			\end{itemize}
	\end{itemize}

	\begin{figure}[H]
		\centering
		\includegraphics[width=7cm]{sample_uv-vis.png}
	\end{figure}

	The unknown concentration of a sample can be determined from a calibration curve constructed from samples of known absorbance and concentration.
	
	Graphing absorbance for standard concentrations produces a calibration curve that can be used to determine the unknown concentration of a solution.
